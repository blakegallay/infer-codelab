...
module TransferFunctions (CFG : ProcCfg.S) = struct
  ...
  type analysis_data = ResourceLeakDomain.summary InterproceduralAnalysis.t
  ...
  let exec_instr (astate : ResourceLeakDomain.t)
    ...
    | Call (_, Direct callee_procname, [actual], _, _loc)
      ...
      | _ ->
          astate )
    | Call (return, Direct callee_procname, HilExp.AccessExpression _allocated :: _, _, _loc) -> (
      ...
      | Some (_callee_proc_desc, callee_summary) ->
          ...
          ResourceLeakDomain.apply_summary astate callee_summary (return, [])
      | None ->
          (* No summary for [callee_procname]; it's native code or missing for some reason *)
          astate )
    | Assign (Base (ret, {Typ.desc= Typ.Tptr ({Typ.desc= Typ.Tstruct ret_typename}, _)}), AccessExpression rhs_access_expr, _loc)
          when Var.is_return ret && is_closeable_typename tenv ret_typename ->
          (* return a resource *)
          (ResourceLeakDomain.record_return_resource astate) |> ResourceLeakDomain.release_resource (HilExp.AccessExpression.to_access_path rhs_access_expr)
...
let checker ({InterproceduralAnalysis.proc_desc} as analysis_data) =
  ...
  Option.map result ~f:(fun post ->
      report_if_leak analysis_data post ;
      ResourceLeakDomain.make_summary post )
